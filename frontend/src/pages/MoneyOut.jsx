...
import { addMoneyOut, fetchSuppliers, fetchExpense, updateMoneyOut, deleteMoneyOut } from "../services/api";
...
const MoneyOut = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "category") {
      const assetDefaults = ["Machinery", "Furniture", "Equipment", "Investments"];
      setForm((prev) => ({
        ...prev,
        category: value,
        depreciationRate:
          (value === "Owner Withdrawals" || assetDefaults.includes(value)) && !prev.depreciationRate
            ? "10"
            : prev.depreciationRate,
      }));
      return;
    }
  ...
  const isAssetCategory = ["Machinery", "Furniture", "Equipment", "Investments"].includes(
    form.category
  );
  const isOwnerWithdrawals = form.category === "Owner Withdrawals";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        depreciationRate:
          isAssetCategory || isOwnerWithdrawals
            ? (form.depreciationRate ? Number(form.depreciationRate) : 10)
            : null,
      };
      if (editingId) {
  ...
  return (
    <div className="page money-out-page">
      ...
      <form className="form card" onSubmit={handleSubmit}>
        ...
        {isOwnerWithdrawals && (
          <>
            <label htmlFor="purchaseDate">Withdrawal Date</label>
            <input
              id="purchaseDate"
              name="purchaseDate"
              type="date"
            ...
            <label htmlFor="depreciationRate">Interest % per year</label>
            <input
              id="depreciationRate"
              name="depreciationRate"
              type="number"
        ...
        <div className="action-row">
          <button className="btn" type="submit" disabled={loading}>
            {loading ? "Saving..." : editingId ? "Update Money Out" : "Save Money Out"}
          </button>
          {editingId && (
            <button
              type="button"
            ...
            <div key={t.id || idx} className="table-row">
              ...
              <div className="table-actions">
                ...
                <button
                  ...
                  onClick={async () => {
                    if (!t?.id) return;
                    const ok = window.confirm("Delete this Money Out entry?");
                    if (!ok) return;
                    try {
